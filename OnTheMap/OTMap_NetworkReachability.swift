//
//  OTMap_NetworkReachability.swift
//  OnTheMap
//
//  Created by knax on 5/3/17.
...
class OTMap_NetworkReachability: NSObject {
    ...
    static func networkReachabilityForInternetConnection() -> OTMap_NetworkReachability? {
        var zeroAddress = sockaddr_in()
        zeroAddress.sin_len = UInt8(MemoryLayout.size( ofValue:zeroAddress))
        
        zeroAddress.sin_family = sa_family_t(AF_INET)
        return OTMap_NetworkReachability(hostAddress: zeroAddress)
    }
    
    static func networkReachabilityForLocalWiFi() -> OTMap_NetworkReachability? {
    ...
    func startNotifier() -> Bool {
        ...
        guard let reachability = networkReachability, SCNetworkReachabilitySetCallback(reachability, { (target: SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) in
            ...
            if let currentInfo = info {
                ...
                if infoObject is OTMap_NetworkReachability {
                    let networkReachability = infoObject as! OTMap_NetworkReachability
